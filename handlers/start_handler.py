...
def build_main_menu(user_region: str = "Не выбран") -> types.InlineKeyboardMarkup:
    keyboard = [
        [types.InlineKeyboardButton(text="Запустить парсинг OLX", callback_data="parse_olx")],
...
def build_link(text: str, href: str) -> str:
    """
    Строит ссылку из текста и href в формате Markdown.
    Пример: [Текст](https://example.com)
    """
    return f"[{text}]({href})"
